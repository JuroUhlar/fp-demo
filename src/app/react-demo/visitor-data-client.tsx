...
export function VisitorDataReactClientDemo() {
  const { data, isLoading, error, getData } = useVisitorData({
    immediate: true,
    linkedId: 'react-demo-page',
  });

  return (
    <div className="space-y-6">
      <div className="rounded-lg border border-gray-200 p-6">
        <h2 className="mb-4 text-xl font-semibold">@fingerprint/react Demo</h2>
        ...
        {data && (
          <div className="space-y-4">
            <div>
              ...
              <code className="rounded bg-gray-100 px-2 py-1 text-sm">{(data as any).visitor_id}</code>
            </div>

            <div>
              <h3 className="mb-2 text-lg font-medium">Event ID</h3>
              <code className="rounded bg-gray-100 px-2 py-1 text-sm">{(data as any).event_id}</code>
            </div>
